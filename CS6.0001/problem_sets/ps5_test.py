...
class ProblemSet5(unittest.TestCase):
    ...
    def test7FilterStories(self):
        ...
        ta = NewsStory('', "asfd New York City asfdasdfasdf", '', '', datetime.now())
        tb = NewsStory('', "asdfasfd new york city! asfdasdfasdf", '', '', datetime.now())
        tnoa = NewsStory('', "something somethingnew york city", '', '', datetime.now())
        tnob = NewsStory('', "something something new york cities", '', '', datetime.now())

        st = DescriptionTrigger("New York City")
        da = NewsStory('', '', "asfd New York City asfdasdfasdf", '', datetime.now())
        db = NewsStory('', '', "asdfasfd new york city! asfdasdfasdf", '', datetime.now())
        dnoa = NewsStory('', '', "something somethingnew york city", '', datetime.now())
        dnob = NewsStory('', '', "something something new york cities", '', datetime.now())

        triggers = [tt, st, self.tt, self.ft]
        stories = [ta, tb, tnoa, tnob, da, db, dnoa, dnob]
        filtered_stories = filter_stories(stories, triggers)
        for story in stories:
            self.assertTrue(story in filtered_stories)
        filtered_stories = filter_stories(stories, [tt, st])
        self.assertEqual(len(filtered_stories), 4)
        filtered_stories = filter_stories(stories, [self.tt])
        self.assertEqual(len(filtered_stories), len(stories))
