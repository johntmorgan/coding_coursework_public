...
def median_sliding_window(nums, k)
  ...
  while true
    if k % 2 == 0
      medians.push((small_heap.max() + large_heap.min()) / 2.0)
    else
      medians.push(small_heap.max())
    end
    ...
    if in_num <= small_heap.max()
      balance += 1
      small_heap.push(in_num)
    else
      balance -= 1
      large_heap.push(in_num)
    end

    if out_num <= small_heap.max()
      balance -= 1
    else
      balance += 1
    end
    ...
    num = hmap[small_heap.max()]
    while !small_heap.empty? && num && num > 0
      hmap[small_heap.max()] -= 1
      small_heap.pop()
      num = hmap[small_heap.max()]
    end

    num = hmap[large_heap.min()]
    while !large_heap.empty? && num && num > 0
      hmap[large_heap.min()] -= 1
      large_heap.pop()
      num = hmap[large_heap.min()]
    end
  end
...
arr = [1,3,-1,2,-2,-3,5,1,5,3]
k = 3
p(median_sliding_window(arr, k))